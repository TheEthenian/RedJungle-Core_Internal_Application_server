...
def read_all_data(db_session,table_name):
    ...
    if table_name == 'hotel':
        res_data = db_session.query(Hotel_Object).all()
        db_session.close()
        return res_data

    if table_name == 'service':
        res_data = db_session.query(Hotel_Service_Object).all()
        db_session.close()
        return res_data

    if table_name == 'config':
        res_data = db_session.query(Hotel_Configuration_Object).all()
        db_session.close()
        return res_data
...
#create_hotel('Cerritos','tenant278','west indiana',754958)
